...
public partial class CompletionProvider
{
    ...
    public static IEnumerable<CompletionItem> GetCompletions(string word, TextEditor textEditor)
    {
        ...
        foreach (CompletionItem completionItem in CompletionItems.Where(completionItem =>
                     completionItem.Name.StartsWith(word)))
        {
        ...
        foreach (Match match in VariableRegex().Matches(text).DistinctBy(x => x.Value))
        {
            ...
            if (!matchValue.StartsWith(word))
            {
                continue;
            }

            if (CompletionItems.Any(x => x.Name == matchValue))
            {
                continue;
            }
