...
public interface IFiles
{
    ...
    void AddCustomTab(object header, Control content, bool select = true, IconSource? icon = null);

    /// <summary>
    ///     Add a new editor tab with the desired content.
    /// </summary>
    Task<OpenedFile?> AddEditorTab(string content, string? path);
    ...
    Task Save(object entity, bool saveAs = false);

    /// <summary>
    ///     Close a lot of tab view items according to the given
    ///     <see cref="FileCloseAction" />.
    /// </summary>
    Task BatchClose(FileCloseAction closeAction);
